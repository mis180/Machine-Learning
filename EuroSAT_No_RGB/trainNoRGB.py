...
from utils.datasetNoRGB import GetDataset
...
if __name__ == "__main__":
    if config.MODEL_NAME=="VGG11":
        ...
        model.features._modules['0'] = nn.Conv2d(10, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        ...
        model = EfficientNet.from_pretrained("efficientnet-b1", in_channels=10, num_classes=10)
        model.to(config.DEVICE)
    elif config.MODEL_NAME == "EFFICIENTNET-B7":
        print("Model name: EFFICIENTNET-B7")
        model = EfficientNet.from_pretrained("efficientnet-b7", in_channels=10,  num_classes=10)
        model.to(config.DEVICE)
    else:
