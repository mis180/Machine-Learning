...
from utils.datasetAllBands import GetDataset
...
train_dataset = GetDataset(
        images_folder='data/EuroSATallBands/',
        path_to_csv='data/EuroSATallBands/train.csv',
        transform=config.train_transforms,)

val_dataset = GetDataset(
        images_folder='data/EuroSATallBands/',
        path_to_csv='data/EuroSATallBands/validation.csv',
        transform=config.val_test_transforms,)

test_dataset = GetDataset(
        images_folder='data/EuroSATallBands/',
        path_to_csv='data//EuroSATallBands/test.csv',
        transform=config.val_test_transforms,)
...
if __name__ == "__main__":
    if config.MODEL_NAME=="VGG11":
        ...
        model.features._modules['0'] = nn.Conv2d(13, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        ...
        model = EfficientNet.from_pretrained("efficientnet-b1", in_channels=13, num_classes=10)
        model.to(config.DEVICE)
        # print(model)
    elif config.MODEL_NAME == "EFFICIENTNET-B7":
        print("Model name: EFFICIENTNET-B7")
        model = EfficientNet.from_pretrained("efficientnet-b7", in_channels=13,  num_classes=10)
        model.to(config.DEVICE)
        # print(model)
    else:
